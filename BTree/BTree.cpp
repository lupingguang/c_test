...
关键字k的插入位置为pt结点的第i个*/
    ...
    while(p!=NULL&&found_tag==0){
        ...
        if(i>0&&p->key[i]==k)                       //找到待查关键字
            found_tag=1;                            //查找成功 
        else{                                       //查找失败 ，由树结构可知以及SearchBTNode返回值可知：
                                                    //  1. i =0，那么比k只可能存在有当前搜索节点的左子节点 p->ptr[0]
                                                    //  2  i >0, 那么比k只可能存在有当前搜索节点的右子节点 p->ptr[i]
            q=p;
            p=p->ptr[i];
        }
    }
    ...
    if(found_tag==1){                               //查找成功
        ...
        r.i=i;
        r.tag=0;
        //printf(" no found k:%d, ptr:%p i:%d",k, r.pt , i);
    }
...
//反映是否在结点p中是否查找到关键字k，而SearchBTree是查找整个树
    if(k<p->key[1]){                                //结点p中查找关键字k失败 
        ...
        while(k<p->key[i]&&i>1)
            i--;
        if(k==p->key[i]){                            //结点p中查找关键字k成功 返回key下表，否则返回下表为0
            return 1;
         }
        else
            return 0;
...
int BTNodeDelete(BTNode *p,KeyType k){
    ...
    if(p==NULL)                                     
        ...
        //分两种情况，最终都是删除的叶子节点的key：
            //1.第一次或者连续递归多次后找到要删除的key位于叶子节点，执行路径是5（5可能递归多次）到4到6
            //2.第一次或者连续递归多次后找到要删除的key位于非叶子节点，执行路径是5（5可能递归多次）到4到1、2、3、5、4、6
        //第6步分两种情况：
            //1.删除叶子节点的key后，该节点大于等于key的最小数量，删除结束
            //2.删除叶子节点的key后，该节点小于key的最小数量，需要向相邻节点借key或者与相邻节点合并
                //借key分如下情况：
                    //1.删除左叶子节点最左侧key，如果右兄弟key足够，则将父节点的第i个key放到左叶子节点最后，并将右叶子节点的key补充到父节点i处,
                    //  否则将右叶子节点加父节点i处key与左节点融合，删除右叶子节点
                    //2.删除左叶子节点最右侧key，如果左兄弟key足够，则将父节点的第i个key放到右叶子节点最后，并将左叶子节点的key补充到父节点i处
                    //  否则将右叶子节点加父节点i处key与左节点融合，删除右叶子节点
                    //3.删除左叶子节点中间位置key，如果左兄弟key足够，则将父节点的第i个key放到右叶子节点最后，并将左叶子节点的key补充到父节点i处
                    //  如果右兄弟key足够，则将父节点的第i个key放到左叶子节点最后，并将右叶子节点的key补充到父节点i处,
                    //  否则将右叶子节点加父节点i处key与左节点融合，删除右叶子节点
        printf("found_tag %d\n",found_tag);
        //在当前节点找到key
        if(found_tag==1){                           //查找成功 
            ...
            if(p->ptr[i-1]!=NULL){                  //1.删除的是非叶子结点
                ...
                BTNodeDelete(p->ptr[i],p->key[i]);  //3.递归执行删除操作，此时是要删除被复制的叶子节点的key，所以一定是found_tag !=1
            }
            else
        ...
        //6.删除叶子节点的key后，由于该节点key数量可能过少，需要做结构调整
        if(p->ptr[i]!=NULL)
...
void Test1(){ 
    ...
    printf("make %d level BTree:\n",m);
    for(j=0;j<n;j++){                               //逐一插入元素 
        s=SearchBTree(t,a[j]);
        if(s.tag==0)
            InsertBTree(t,s.i,a[j],s.pt);
        printf("  step %d, insert element:%d\n ",j+1,a[j]);
        PrintBTree(t);
    }
    ...
    printf("recursive release BTree:\n");                       //递归释放B树
    DestroyBTree(t);                                 
    PrintBTree(t);
} 
